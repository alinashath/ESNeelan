...
## ES Neelan (auction app) mapping

The native app and web shell follow this document with **Inter** (`@expo-google-fonts/inter`) as the cross-platform substitute for SF Pro. Semantic tokens in `src/theme/tokens.ts` mirror the YAML above, plus:

| Concern | Token / pattern |
|---|---|
| Live / success | `colors.success` / `colors.secondary` — `#2ECC8A` (only semantic accent beyond Action Blue) |
| Ending soon / error / destructive | `colors.danger` — `#D92D20` |
| Primary CTA & links on light surfaces | `colors.primary` / `colors.accent` — `#0066CC` (`palette.primary`) |
| Focus ring root | `palette.primaryFocus` — `#0071E3` (`accentBorderStrong`, focus affordances) |
| Links on dark tiles / scrims | `palette.primaryOnDark` — `#2997FF` where Action Blue lacks contrast |
| Page wash | `colors.background` — parchment `#F5F5F7` (`palette.canvasParchment`) |
| Cards & sheets | `colors.surfaceMuted` — white `#FFFFFF` (`palette.canvas`) |
| Chrome shadows | Minimal RN elevation; **no** duplicate “product shadow” on flat cards — reserve strong shadow for hero product imagery only (`shadows.productImage` in tokens). |

Legacy exports `goldBorderSubtle` / `goldWash` remain as **blue-tint** aliases for older imports.
